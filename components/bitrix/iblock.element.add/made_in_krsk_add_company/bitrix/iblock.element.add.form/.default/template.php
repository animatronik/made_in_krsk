				...
				<tr class="tr_<?=$propertyID;?>">
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								?>
								<input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" id="UF_<?=$propertyID?>" size="25" value="<?=$value?>" class="input_text_style" /><br /><?
					...
					[<a href="<?=$arResult["ELEMENT_FILES"][$value]["SRC"]?>"><?=GetMessage("IBLOCK_FORM_FILE_DOWNLOAD")?></a>]<br />
							...
							<input type="<?=$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
									...
									case "multiselect":
									?>
							<select name="PROPERTY[<?=$propertyID?>]<?=$type=="multiselect" ? "[]\" size=\"".$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]."\" multiple=\"multiple" : ""?>" style="<?if ($propertyID='IBLOCK_SECTION') {echo "display:none;";}?>">
								<option value=""><?echo GetMessage("CT_BIEAF_PROPERTY_VALUE_NA")?></option>																										<!--Скрываем поля разделов-->
									<?
										...
										foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum)
										{
											...
											//проверка привязки разделов инфоблока и выбор
											if (in_array($key,$arResult["USER_BIND_SECTIONS"]))
											{												
												$checked = true;
											}
											?>
...
<?$APPLICATION->IncludeComponent("bitrix:map.yandex.search", "made_in_krsk_location", Array(
	...
	"MAP_HEIGHT" => "250",	// Высота карты
	"MAP_DATA" => "a:3:{s:10:\"yandex_lat\";d:56.01056899999343;s:10:\"yandex_lon\";d:92.85254499999999;s:12:\"yandex_scale\";i:10;}",	// Начальная позиция карты
	...
	"MAP_ID" => "yam_1",	// Идентификатор карты
	),
	false
);?>
